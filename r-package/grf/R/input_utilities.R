...
create_train_matrices <- function(X,
  ...
  X <- as.matrix(X)
  out[["train.matrix"]] <- as.matrix(cbind(X, outcome, treatment, instrument, running_var,
                                          survival.numerator, survival.denominator, censor, sample.weights))
